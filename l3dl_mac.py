...
def download(keys):
    if mpd_url != None:
        ...
        if select_ and not os.path.exists(file_path):
            os.system(f'N_m3u8DL-RE "{mpd_url}" --log-level ERROR --binary-merge --live-real-time-merge --mp4-real-time-decryption --key {keys} -M format=mkv:muxer=mkvmerge -R 30M --del-after-done false --tmp-dir "{base_dir}" --save-name "{name}" --save-dir "{base_dir}" --use-shaka-packager -mt TRUE --thread-count 10', shell=True)
        elif not select_ and not os.path.exists(file_path):
            print(
                f'Processing {name} | Download, decrypt, and muxing may take some time.')
            subprocess.call(f'N_m3u8DL-RE "{mpd_url}" --log-level ERROR --binary-merge --live-real-time-merge --mp4-real-time-decryption --key {keys} -M format=mkv:muxer=mkvmerge -R 30M --del-after-done false -ss all -sa best -sv best --tmp-dir "{base_dir}" --save-name "{name}" --save-dir "{base_dir}" --use-shaka-packager -mt TRUE --thread-count 10', shell=True)
        else:
